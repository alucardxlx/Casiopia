...
namespace Server.Network
{
	...
	{
		public VendorSellList( Mobile shopkeeper, Dictionary<Item, SellItemState> table ) : base( 0x9E )
		{
			this.EnsureCapacity( 256 );
			...
			foreach ( KeyValuePair<Item, SellItemState> kvp in table )
			{
				SellItemState state = kvp.Value;
	...
	public sealed class DisplayEquipmentInfo : Packet
	{
		public DisplayEquipmentInfo( Item item, EquipmentInfo info ) : base( 0xBF )
		{
			...
			this.EnsureCapacity( 17 + (info.Crafter == null ? 0 : 6 + info.Crafter.RawName == null ? 0 : info.Crafter.RawName.Length) + (info.Unidentified ? 4 : attrs.Length * 6) );
	...
	{
	    public FlashEffect( FlashType flash ) : base( 0x70, 28 )
	    {
		   m_Stream.Write( (byte) 0x4 ); //effectType
		   m_Stream.Write( (int) 0 ); //fromSerial
		   m_Stream.Write( (int) 0 ); //toSerial
		   m_Stream.Write( (ushort) flash ); //in regular 0x70 ItemID is here
	...
	public sealed class SupportedFeatures : Packet
	{
		...
		public SupportedFeatures( NetState ns ) : base( 0xB9, ns.ExtendedSupportedFeatures ? 5 : 3 )
		{
			...
			if ( acct != null && acct.SFO != 0 )
				flags = acct.SFO; //Supported Features Override used for OldClient command.

			flags |= m_AdditionalFlags;

			if ( acct != null && acct.Limit >= 6 )
			{
				flags |= FeatureFlags.EleventhAgeSplash; //AOS Required though?
	...
	{
		public NewMobileAnimation( Mobile m, int action, int frameCount, int variation ) : base( 0xE2, 10 )
		{
			m_Stream.Write( (int) m.Serial );
			m_Stream.Write( (short) action );
			m_Stream.Write( (short) frameCount );
			m_Stream.Write( (byte) variation );
		}
	}
	...
	public sealed class CharacterListOld : Packet
	{
		public CharacterListOld( IAccount a, CityInfo[] info ) : base( 0xA9 )
		{
			...
			m_Stream.Write( (int)(flags | CharacterListOld.AdditionalFlags) ); // Additional Flags
		}
